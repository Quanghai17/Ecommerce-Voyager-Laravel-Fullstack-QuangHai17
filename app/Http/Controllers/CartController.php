...
class CartController extends Controller
{
    ...
    public function store(Request $request) {
        ...
        if($duplicates->isNotEmpty()) {
            session()->flash('success', 'Sản phẩm đã có trong giỏ hàng');
            return redirect()->route('cart.index');
        } else if($duplicatesLater->isNotEmpty()) {
            Cart::instance('saveForLater')->remove($duplicatesLater->first()->rowId);
        }
        // dd(request()->all());
        Cart::instance('default')->add(request()->id, request()->name, $request->qty, request()->price)->associate('App\Product');
        session()->flash('success', 'Thêm vào giỏ hàng thành công');
        return redirect()->route('cart.index');
    }

    //Click vào vao ngay sẽ đi vào thanh toán luôn
